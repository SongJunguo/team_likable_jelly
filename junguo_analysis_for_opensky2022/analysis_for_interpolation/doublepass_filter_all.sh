...
# 批量运行“二次三点投票（classic_dp）”过滤，生成双过滤后的日文件。
# 特点：
# - 并行按天处理（HDD 建议并发 6–8）
...
OUT_DIR="/workspace/aircraft_trajectory/team_likable_jelly/opensky_2024_PRC_dataset/classic_filtered_trajectories__doublepass_v1"
PROCS=8
...
run_one() {
  ...
  if [[ "$DRYRUN" == "1" ]]; then
    echo "DRYRUN python filter_trajs.py -t_in $in_f -t_out $out_f -strategy classic_dp" | tee -a "$log"
    return 0
  fi
  python filter_trajs.py -t_in "$in_f" -t_out "$out_f" -strategy classic_dp >>"$log" 2>&1 \
    || { echo "❌ 失败: $d (详见 $log)"; return 1; }
}

export -f run_one
export IN_DIR OUT_DIR DRYRUN
