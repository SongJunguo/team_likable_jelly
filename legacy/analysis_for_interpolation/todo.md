...
然后再把每个小段重采样到标准的1hz，再线性插值，可以考虑使用带平滑的pipelines/clean_segment/interpolate.py
...
过滤脚本为什么不在已有的pipelines/clean_segment/filter_trajs.py上新增呢？不过现在的项目太混乱了，尤其是legacy/analysis_for_interpolation下面，新建一个或许也可以？不过原有修改也很好。
阶段2：修改切分脚本会不会影响现有的流程？我在使用legacy/analysis_for_interpolation/run_full_pipeline_with_interpolate.sh。你帮我检查一下。
...
# 第五次问答
  ...
  3. 修复run_fast_pipeline.sh
    ...
    - 这样可以批量处理全年数据
  4. 全量运行（验证通过后）
  bash pipelines/clean_segment/run_fast_pipeline.sh
...
(opensky) root@ea820f0b5965:/workspace/aircraft_trajectory/team_likable_jelly# 
  pipelines/clean_segment/run_fast_pipeline_parallel.sh
  pipelines/clean_segment/run_fast_pipeline.sh
  pipelines/clean_segment/run_staged_pipeline.sh\
  ...
  # 第八次提问
  > pipelines/clean_segment/config.sh\\
...
legacy/analysis_for_interpolation/run_full_pipeline_with_interpolate.sh这个会好一些，轨迹不很稀碎。
