...
#   bash legacy/analysis_for_interpolation/doublepass_filter_all.sh \
...
OUT_DIR="/workspace/aircraft_trajectory/team_likable_jelly/opensky_2024_PRC_dataset/classic_filtered_trajectories_doublepass_loop_v8"
PROCS=41
...
run_one() {
  ...
  if [[ "$DRYRUN" == "1" ]]; then
    echo "DRYRUN python -m pipelines.clean_segment.filter_trajs -t_in $in_f -t_out $out_f -strategy $STRATEGY" | tee -a "$log"
    return 0
  fi
  python -m pipelines.clean_segment.filter_trajs -t_in "$in_f" -t_out "$out_f" -strategy "$STRATEGY" >>"$log" 2>&1 \
