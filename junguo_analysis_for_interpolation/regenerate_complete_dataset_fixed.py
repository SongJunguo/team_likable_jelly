#!/usr/bin/env python3
"""
修复版本的 regenerate_complete_dataset.py

主要修复：
1. 在插值后添加速度检查，删除超速的插值点
2. 循环迭代直到没有新的超速点
"""
...
# 速度校验阈值（米/秒）
MAX_SPEED_MPS = 600.0  # 600 m/s ≈ 2160 km/h，留一定余量
...
def check_and_remove_overspeed(df: pd.DataFrame, max_speed_mps: float = MAX_SPEED_MPS, max_iterations: int = 10) -> tuple[pd.DataFrame, dict]:
    """检查并删除超速点（循环迭代直到收敛）
...
class CompleteDatasetGenerator:
    ...
    def process_trajectory(self, df):
        ...
        # **关键修复：速度检查**
        # 删除插值后产生的超速点
        df_interpolated, speed_check_stats = check_and_remove_overspeed(
            df_interpolated,
...
def process_single_file(args):
    ...
    try:
        ...
        for i, (flight_id, group) in enumerate(flight_groups):
            ...
            if result['success'] and not processed_traj.empty:
                ...
                if 'speed_check' in result:
                    stats['total_speed_removed'] += result['speed_check']['removed_points']
        ...
        if processed_data:
            ...
            print(f"文件 {date_file} 处理完成，保存了 {stats['success_trajectories']} 条轨迹，删除了 {stats['total_speed_removed']} 个超速点")
        else:
...
def main():
    ...
    # 数据目录
    input_dir = "/workspace/aircraft_trajectory/team_likable_jelly/opensky_2024_PRC_dataset/classic_filtered_trajectories_doublepass_loop_v7"
    output_dir = "complete_high_quality_trajectories_fixed"
    ...
    total_speed_removed = sum(r['total_speed_removed'] for r in results)
    ...
    print(f"速度检查删除点数: {total_speed_removed:,}  ✨ 新增")
    ...
    # 生成详细报告
    report_file = f"complete_dataset_regeneration_report_fixed_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
    with open(report_file, 'w', encoding='utf-8') as f:
        f.write("完整高质量轨迹数据集重新生成报告（修复版）\n")
        f.write("=" * 50 + "\n\n")
        f.write(f"生成时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
        f.write(f"输出目录: {output_dir}\n")
        f.write(f"修复内容: 添加速度检查，删除超过 {MAX_SPEED_MPS} m/s ({MAX_SPEED_MPS*3.6:.0f} km/h) 的插值点\n\n")
        ...
        f.write(f"  速度检查删除点数: {total_speed_removed:,}\n")
        ...
        for r in results:
            if r['found_trajectories'] > 0:
                f.write(f"  {r['date']}: 找到{r['found_trajectories']}条, 成功{r['success_trajectories']}条, 删除超速点{r['total_speed_removed']}个\n")
    ...
    if total_success > 0:
        ...
        print(f"✨ 速度修复: 删除了 {total_speed_removed:,} 个超速插值点")
    else:
